...
(function() {
    ...
    signCSS += "#getDown,#allsign,#newbutn,#newbutn2,#newbutn3{font-size:12px; background:rgba(228,228,228,0.4); cursor:pointer; margin:0,0,0,1px; padding:0px 3px;color:black; border:2px ridge black;}";
    //signCSS += '#autoSign{margin-right:1px;}';
    ...
    if (window.location.href.indexOf("//www.baidu.com/") != -1) {
        ...
        if (headLine) {
            ...
            addStyle("#allSign{margin:9px 31px 0 0;} #autoSign{margin:2px 1px 0 0}");
        }
    ...
    function jjuds() {
        ...
        let baNameF = []; //吧名收集数组
        let heer = new Date();
        let sed = heer.getTime() - 600000;

        function getTieba(nn, lp) { //获取第2-第n页的贴吧列表
            let addTail = "&pn=" + nn;
            let urll = "http://tieba.baidu.com/f/like/mylike?v=" + sed + addTail;
            setTimeout(function() { //延时操作以免度娘误会
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: function(reText) {
                        let reTextTxt = reText.responseText.replace(/[	]/g, "").replace(/<td>\r\n/g, "<td>").replace(/\r\n<\/td>/g, "</td>").replace(/<span.*?span>\r\n/g, "");
                        let ww = reTextTxt.match(/<a[ ]href=".*?(?=<\/a><\/td>)/g);
                        if (ww != null) {
                            let s, qq, dd, ns;
        ...
        GM_xmlhttpRequest({ //从“我的贴吧”第1页获取列表
            ...
            onload: function(reDetails) {
                let simTxt = reDetails.responseText.replace(/[	]/g, "").replace(/<td>\r\n/g, "<td>").replace(/\r\n<\/td>/g, "</td>").replace(/<span.*?span>\r\n/g, "");
                ...
                let qqee = simTxt.match(/<a[ ]href=.*pn=(\d+)">尾页<\/a>/);
                if (qqee) { //检查是否分页，分页则继续获取贴吧名单
                    let deho = Number(qqee[1]);
