// ==UserScript==
// @name           tiebaAllsign(2020update)
// @description    贴吧集中签到(2020修正版)
...
(function() {
    ...
    // 更新说明
    let umdisplayedkey = 'update_message_displayed_' + GM_info.script.version;
    let umdisplayed = GM_getValue(umdisplayedkey, false);
    let update_msg = (function(){/*2020.10.07.1 - 贴吧集中签到 - 更新说明
    本次更新为非必需更新，是为解决少数用户自动签到时出现的自动退出登陆的状况而存在的更新版本，如果您在使用中没有遇到过百度账号自动退出登陆的情况，可以忽略本提示并继续使用tiebaAllsign进行日常贴吧签到。
    如果您过去在使用中遇到了以下情况中的一个或多个，请在继续使用前务必仔细阅读本说明：
        ·全签到卡在“开始签到准备，正在获取贴吧列表第1页”并不能继续
        ·全签到失败，刷新页面后百度账号自动退出登陆
    出现上述问题原因是：浏览器在刚刚登陆了百度账号后，就立即进行了全签到
    解决方案：重新登录您的百度账号，并不要立即进行全签到；先手动访问一下贴吧首页(http://tieba.baidu.com/)，再点击按钮进行全签到。之后只要您不再主动退出您的百度账号，就不会再出现此问题，即使再次出现问题，也可以重复上述方法解决
    自动化的解决方案本人正在研究中，请耐心等待，谢谢
    */}).toString().replace(/function\(\){\/\*/, '').replace(/\n*\s*\*\/}/g, '');
    if (!umdisplayed) {
        //alert(update_msg);
        GM_setValue(umdisplayedkey, true);
    }
    ...
    // 检查是否未登录百度账号
    if (document.querySelector('.s-top-login-btn')) {
        newEm.innerHTML = "请先登录";
        newEm.title = '登录百度账号后即可使用全签到';
        disableSign();
        logon = false;
    }
    ...
    function disableSign() {
        const newsignCss = document.createElement("style");
        ...
        document.head.appendChild(newsignCss);
        return newsignCss;
    }
    ...
    function jjuds() {
        ...
        let newsignCss = disableSign(); // 签到过程中，禁用签到按钮
        ...
        function requestTieba(pn) {
            console.log('requestTieba start');
            let url = "http://tieba.baidu.com/f/like/mylike?v=" + String(date.getTime() - 600000) + "&pn=" + String(pn);
            console.log('url = ' + url)
            GM_xmlhttpRequest({
                method: 'GET',
                synchronous: false,
                ...
                headers: { // 添加http头信息，希望有用
                    ...
                    "cookie": document.cookie, //encodeURIComponent(document.cookie)
                    "Upgrade-Insecure-Requests": "1"
                },
                onload: function(request) {
                    console.log('requestTieba onload');
                    getTieba(request.responseText);
                    if (pn < maxPage) { // 只要没到最后一页，就不要停下来啊啊啊～～～～
                        setTimeout(function() {requestTieba(pn + 1);}, 1000)
                    } else {
                        gowork(allAncs, baNameF);
                    }
                },
                /*onreadystatechange: function() {
                    alert("requestTieba onreadystatechange");
                },*/
                onerror: function() {
                    prompt('由于度娘的神奇操作，签到过程中发生了错误并被度娘退出了登录:(\n请在重新登录后手动访问一下贴吧主页http://tieba.baidu.com/，再点击全签到按钮进行全签到\n贴吧主页链接为：', "http://tieba.baidu.com/");
                }
            })
        }
    }
